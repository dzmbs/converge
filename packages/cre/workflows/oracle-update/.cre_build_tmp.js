...
var configSchema = exports_external.object({
  ...
  issuerApiUrl: exports_external.string(),
  deviationThresholdBips: exports_external.number(),
...
var fetchIssuerRate = (sendRequester, config) => {
...
function rateToFixed(rate) {
  return BigInt(Math.round(rate * 1000000000000000000));
}
function deviationExceeds(oldRate, newRate, thresholdBips) {
  if (oldRate === 0n)
    return true;
...
function updateChain(runtime2, evmConfig, newRateFixed, fetchedRate) {
  ...
  try {
    ...
    const onChainResult = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: ZERO_ADDRESS,
